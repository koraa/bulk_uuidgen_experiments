...
extern "C" __attribute__((flatten)) int main() {
  const size_t buf_size = getpagesize() * 16;
  ...
  while (true) {
    // Convert to strings
    for (size_t ix = (buf_size - off) / uuid_line_len; ix > 0; ix--)
      write_uuid(uuid_rng(), buf, off);


    size_t rest = buf_size - off;

    // No overhanging uuid (buffer completely filled)
    if (rest == 0) {
      flush();
      continue;
    }

    // Deal with partial uuid at the end/beginning of the block
    // Generate the overhanging uuid
    size_t smallbuf_off = 0;
    std::array<char, uuid_line_len> smallbuf;
    write_uuid(uuid_rng(), std::data(smallbuf), smallbuf_off);

    // Copy the initial part of the uuid into the buffer
    std::copy_n(std::begin(smallbuf), rest, buf+off);
    flush();

    // Copy the rest of the overhanging uuid into the next buffer
    std::copy(std::begin(smallbuf)+rest, std::end(smallbuf), buf);
    off += (uuid_line_len - rest);
  }
